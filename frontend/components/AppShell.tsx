...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = {
    candidate: [
      ...
      { href: '/dashboard/candidate#jobs', label: 'Find Jobs & Matches' },
      { href: '/dashboard/candidate#resume', label: 'Resume Rating' },
      { href: '/profile/candidate', label: 'My Profile Settings' },
    ],
    recruiter: [
      { href: '/dashboard/recruiter', label: 'Overview' },
      { href: '/dashboard/recruiter#jobs', label: 'My Job Listings' },
      { href: '/dashboard/recruiter#post', label: 'Post a New Job' },
      { href: '/profile/recruiter', label: 'Company Profile' },
    ],
    admin: [
      { href: '/dashboard/admin', label: 'Overview' },
      { href: '/dashboard/admin#users', label: 'User Registry' },
      { href: '/dashboard/admin#logs', label: 'System Audit Logs' },
      { href: '/profile/admin', label: 'Admin Settings' }
    ]
  };

  const currentNav = navItems[role as keyof typeof navItems] || navItems.candidate;
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 flex flex-col md:flex-row antialiased font-sans">
      ...
      `}>
        <div className="flex flex-col flex-1 py-6 px-4 gap-6 overflow-y-auto">
          ...
          <div className="px-3 py-2 bg-zinc-50 border border-zinc-150 rounded-lg flex flex-col gap-1">
            <span className="text-[10px] text-zinc-400 uppercase font-semibold tracking-widest">Active Workspace</span>
        ...
        <div className="p-4 border-t border-zinc-200 bg-zinc-50 flex flex-col gap-3">
          <div className="flex items-center gap-3 px-1">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-zinc-500 truncate">{user?.email || 'user@email.com'}</span>
            </div>
          </div>
          <button
      ...
      {mobileMenuOpen && (
        <div 
          ...
          className="fixed inset-0 bg-black/20 z-40 md:hidden backdrop-blur-xs transition-opacity duration-200"
        />
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex items-center justify-between px-8 py-4 bg-white border-b border-zinc-200 sticky top-0 z-35">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold uppercase text-zinc-400 tracking-wider">SkillMatch Workspace</span>
            <span className="text-zinc-300">/</span>
            <span className="text-sm font-medium text-zinc-800">{roleLabel} Panel</span>
          </div>
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-2 border border-zinc-200 px-3 py-1.5 rounded-full bg-zinc-50 text-zinc-600">
